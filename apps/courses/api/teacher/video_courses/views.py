...
class CourseListCreateView(generics.ListCreateAPIView):
    ...
    def get_permissions(self):
        # Teacher endpoint always requires authentication
    ...
    def get_queryset(self):
        ...
        logger.info(f"🎥 VIDEO DEBUG - view_mode: {view_mode}, authenticated: {self.request.user.is_authenticated}, user: {getattr(self.request.user, 'email', 'Anonymous')}, role: {getattr(self.request.user, 'role', None)}")
        ...
        if view_mode == 'teacher_courses' and self.request.user.is_authenticated and getattr(self.request.user, 'role', None) == 'teacher':
            ...
            logger.info(f"🎥 VIDEO TEACHER MODE - Found {queryset.count()} video courses for teacher {self.request.user.email}")
        else:
            ...
            logger.info(f"🎥 VIDEO PUBLIC MODE - Found {queryset.count()} published video courses")
