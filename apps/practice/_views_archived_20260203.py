...
from .permissions import IsTeacher, IsTeacherOrReadOnly, IsCourseOwner
from .throttling import (
    ChallengeProgressThrottle,
    CourseCreationThrottle,
    ContentCreationThrottle,
    HeartsRefillThrottle
)
...
# Import Vapi views (now at practice/vapi_views.py)
...
from .models import (
    ...
    # NOTE: AI Speaking/Listening models removed - they don't exist in models.py
    # If needed in future, create these models first:
    # SpeakingExercise, SpeakingSession, SpeakingTurn, SpeakingProgress,
    # ListeningExercise, ListeningSession, ListeningAttempt, ListeningProgress, AudioSegment
)
...
# =============================================================================
# STUB DEFINITIONS FOR REMOVED MODELS/SERIALIZERS
# These models/serializers don't exist in the codebase. The views that use them
# will fail at runtime, but the module can at least be imported.
# TODO: Either create these models or remove the views that depend on them.
# =============================================================================


class _StubQuerySet:
    """Fake QuerySet that returns empty results"""
...
class _StubManager:
    """Fake Manager that returns StubQuerySet"""
    def filter(self, *args, **kwargs):
    ...
    def get(self, *args, **kwargs):
        raise Exception("Stub model - does not exist")
...
class _StubModel:
    """Fake Model class with objects manager"""
    objects = _StubManager()


# Stub models (don't exist in models.py) - using _StubModel to avoid import errors
SpeakingExercise = _StubModel
SpeakingSession = _StubModel
SpeakingTurn = _StubModel
SpeakingProgress = _StubModel
ListeningExercise = _StubModel
ListeningSession = _StubModel
ListeningAttempt = _StubModel
ListeningProgress = _StubModel
AudioSegment = _StubModel

# Stub serializers (depend on non-existent models)
SpeakingExerciseSerializer = None
SpeakingExerciseListSerializer = None
SpeakingSessionSerializer = None
SpeakingSessionCreateSerializer = None
SpeakingTurnSerializer = None
SpeakingTurnCreateSerializer = None
SpeakingProgressSerializer = None
SpeakingAnalysisSerializer = None
SpeakingStatsSerializer = None
QuickSpeakingFeedbackSerializer = None
ListeningExerciseSerializer = None
ListeningExerciseListSerializer = None
ListeningSessionSerializer = None
ListeningSessionCreateSerializer = None
ListeningAttemptSerializer = None
ListeningAttemptCreateSerializer = None
ListeningProgressSerializer = None
ListeningAnalysisSerializer = None
ListeningStatsSerializer = None
AudioSegmentSerializer = None
QuickListeningFeedbackSerializer = None
...
class CoursesListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        """
        List courses with optional statistics.

        Query params:
        - include_drafts: Include draft courses (for teachers)
        - include_stats: Include unit/lesson/challenge counts
        """
        ...
        for course in courses:
            course_data = {
                'id': str(course.id),
                'courseId': str(course.id),  # Alias for frontend compatibility
                'title': course.title,
                ...
                'teacher': str(course.teacher.id) if course.teacher else None,
                'teacherId': str(course.teacher.id) if course.teacher else None,
                'teacherName': course.teacherName or '',
            ...
            if include_stats:
                try:
                    # Count units using direct query (most reliable)
                    units_count = PracticeUnit.objects.filter(course_id=course.id).count()

                    # Count lessons across all units
                    lessons_count = PracticeLesson.objects.filter(
                        unit__course_id=course.id
                    ).count()

                    # Count challenges across all lessons
                    challenges_count = PracticeChallenge.objects.filter(
                        lesson__unit__course_id=course.id
                    ).count()
                    ...
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.error(f"Error calculating stats for course {course.id}: {str(e)}")
                    course_data.update({
                        'units_count': 0,
                        'lessons_count': 0,
                        'challenges_count': 0,
...
class ChallengeProgressView(APIView):
    ...
    - SELECT/ASSIST: Simple option selection
    - FILL_BLANK/TRANSLATION: Text input with typo tolerance
    - MATCH_PAIRS: Pair matching validation
    - SENTENCE_ORDER: Word order validation
    - LISTENING: Audio-based option selection
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        # Extended parameters for complex challenge types
        text_answer = request.data.get('text_answer')  # For FILL_BLANK/TRANSLATION
        ordered_options = request.data.get('ordered_options')  # For SENTENCE_ORDER
        paired_options = request.data.get('paired_options')  # For MATCH_PAIRS

        logger.info(f"Challenge ID: {challenge_id}, Selected Option: {selected_option_id}")
        logger.info(f"Extended data - text_answer: {text_answer}, ordered_options: {ordered_options}, paired_options: {paired_options}")
        ...
        try:
            ...
            # Get all options for the challenge
            all_options = list(ChallengeOption.objects.filter(challenge=challenge).order_by('order'))

            # For simple selection types, we need selected_option_id
            if challenge.type in ['SELECT', 'ASSIST', 'LISTENING', 'TRUE_FALSE'] and selected_option_id:
        ...
        # ================================================================
        # VALIDATION LOGIC BY CHALLENGE TYPE
        # ================================================================
        ...
        # --- FILL_BLANK and TRANSLATION: Text input validation with typo tolerance ---
        if challenge.type in ['FILL_BLANK', 'TRANSLATION']:
            if text_answer:
                ...
                if correct_options:
                    ...
                    for correct_opt in correct_options:
                        normalized_correct = normalize_text(correct_opt.text)

                        # Exact match (case-insensitive)
                        if normalized_answer == normalized_correct:
                            is_correct = True
                            logger.info(f"FILL_BLANK/TRANSLATION: Exact match for '{text_answer}'")
                            break

                        # Typo tolerance: 90% similarity threshold
                        similarity = similarity_ratio(text_answer, correct_opt.text)
                        if similarity >= 0.90:
                            is_correct = True
                            logger.info(f"FILL_BLANK/TRANSLATION: Typo tolerance match ({similarity:.2%}) for '{text_answer}'")
                            break

                        logger.info(f"FILL_BLANK/TRANSLATION: No match. Similarity: {similarity:.2%}")
            elif selected_option_id:
                # Fallback to option selection
                try:
                    selected_option = ChallengeOption.objects.get(id=selected_option_id, challenge=challenge)
        ...
        # --- SENTENCE_ORDER: Validate word order ---
        elif challenge.type == 'SENTENCE_ORDER':
            if ordered_options and isinstance(ordered_options, list):
                # Get the correct order (options sorted by 'order' field)
                correct_order = [str(opt.id) for opt in all_options]
                submitted_order = [str(opt_id) for opt_id in ordered_options]

                logger.info(f"SENTENCE_ORDER: Correct order: {correct_order}")
                logger.info(f"SENTENCE_ORDER: Submitted order: {submitted_order}")

                if correct_order == submitted_order:
                    is_correct = True
                    logger.info("SENTENCE_ORDER: Order is correct!")
                else:
                    is_correct = False
                    logger.info("SENTENCE_ORDER: Order is incorrect")
            else:
                logger.warning("SENTENCE_ORDER: No ordered_options provided")
                is_correct = False

        # --- MATCH_PAIRS: Validate pair matching ---
        elif challenge.type == 'MATCH_PAIRS':
            if paired_options and isinstance(paired_options, dict):
                # Options are stored as "English - Portuguese" format
                # paired_options format: { "option_id": "portuguese_text", ... }
                all_correct = True

                for opt in all_options:
                    opt_id = str(opt.id)
                    if opt_id in paired_options:
                        # Parse the expected format "English - Portuguese"
                        ...
                        if len(parts) == 2:
                            ...
                            if expected_portuguese != submitted_portuguese:
                                ...
                                similarity = similarity_ratio(parts[1], paired_options[opt_id])
                                if similarity < 0.90:
                                    all_correct = False
                            ...
                            all_correct = False
                            logger.warning(f"MATCH_PAIRS: Invalid option format: '{opt.text}'")
                            break
                    else:
                        all_correct = False
                        logger.info(f"MATCH_PAIRS: Missing pair for option {opt_id}")
                        break

                is_correct = all_correct
                logger.info(f"MATCH_PAIRS: Result is_correct={is_correct}")
            else:
                logger.warning("MATCH_PAIRS: No paired_options provided")
                is_correct = False

        # --- SELECT, ASSIST, LISTENING, TRUE_FALSE: Simple option selection ---
        elif challenge.type in ['SELECT', 'ASSIST', 'LISTENING', 'TRUE_FALSE', 'SPEAKING']:
            if selected_option:
                is_correct = selected_option.is_correct
            else:
                is_correct = False
        ...
        logger.info(f"Final validation result: is_correct={is_correct}")
...
class RefillHeartsView(APIView):
    ...
    SECURITY: Rate limited to prevent abuse of the hearts system.
    """
    permission_classes = [IsAuthenticated]
    throttle_classes = [HeartsRefillThrottle]
...
class PracticeUnitCreateView(generics.ListCreateAPIView):
    ...
    SECURITY: Read operations (GET) allowed for all authenticated users.
    Write operations (POST) require teacher role.
    """
    ...
    def get_queryset(self):
        ...
        OPTIMIZED: Uses select_related and annotate to avoid N+1 queries.
        """
        queryset = super().get_queryset().select_related('course').annotate(
            lessons_count_annotated=Count('lessons')
        )
    ...
    def list(self, request, *args, **kwargs):
        ...
        OPTIMIZED: Uses pre-annotated lessons_count from get_queryset.
        """
        ...
        for unit in queryset:
            units_data.append({
                ...
                'order': unit.order,
                'course_id': str(unit.course_id),  # Use FK directly, avoid extra query
                'lessons_count': unit.lessons_count_annotated  # Use annotated value
            })
...
class PracticeUnitUpdateView(generics.RetrieveUpdateDestroyAPIView):
    ...
    SECURITY: Read operations (GET) allowed for all authenticated users.
    Write operations (PUT/PATCH/DELETE) require teacher role.

    OPTIMIZED: Uses select_related to avoid N+1 on course FK.
    """
    queryset = PracticeUnit.objects.select_related('course').all()
    serializer_class = PracticeUnitSerializer
    permission_classes = [IsAuthenticated, IsTeacherOrReadOnly]
    lookup_field = 'id'
...
class PracticeLessonUpdateView(generics.RetrieveUpdateDestroyAPIView):
    ...
    OPTIMIZED: Uses select_related to avoid N+1 on unit FK.
    """
    queryset = PracticeLesson.objects.select_related('unit', 'unit__course').all()
    serializer_class = PracticeLessonSerializer
    permission_classes = [IsAuthenticated, IsTeacherOrReadOnly]
    lookup_field = 'id'
    lookup_url_kwarg = 'lesson_id'
...
class PracticeChallengeUpdateView(generics.RetrieveUpdateDestroyAPIView):
    ...
    OPTIMIZED: Uses select_related and prefetch_related to avoid N+1.
    """
    queryset = PracticeChallenge.objects.select_related(
...
class ChallengeOptionCreateView(generics.CreateAPIView):
    ...
    SECURITY: Only teachers can create challenge options.
    """
...
# NOTE: mark_notification_read and mark_achievement_celebrated functions
# are defined later in this file (around line 4700+) in the Student Achievement section.
# Duplicate definitions were removed during cleanup.
...
class ListeningSessionCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        # ATOMIC: Ensure session creation and progress update succeed together
...
class GenerateReferenceAudioView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            print(f"✅ TTS generated {len(audio_bytes)} bytes for text: {text[:50]}...")
            ...
            if save_to_s3:
                try:
                    ...
                    print(f"✅ Audio uploaded to S3: {filename} ({len(audio_bytes)} bytes)")
